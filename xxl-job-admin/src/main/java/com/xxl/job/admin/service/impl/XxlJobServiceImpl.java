...
public class XxlJobServiceImpl implements XxlJobService {
    ...
    public ReturnT<String> add(XxlJobInfo jobInfo) {
        ...
        } else if (scheduleTypeEnum == ScheduleTypeEnum.FIX_DATE) {
            if (jobInfo.getScheduleConf() == null) {
                return new ReturnT<String>(ReturnT.FAIL_CODE, (I18nUtil.getString("schedule_type")));
            }
            if (!jobInfo.getScheduleConf().matches("\\d{4}-\\d{2}-\\d{2}\\s\\d{2}:\\d{2}:\\d{2}")) {
                return new ReturnT<>(ReturnT.FAIL_CODE, (I18nUtil.getString("system_unvalid")));
            }
            //当前时间比要配置的时间晚
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
            try {
                if (sdf.parse(jobInfo.getScheduleConf()).getTime() < System.currentTimeMillis()) {
                    return new ReturnT<>(ReturnT.FAIL_CODE, (I18nUtil.getString("system_unvalid")));
                }
            } catch (Exception e) {
                return new ReturnT<>(ReturnT.FAIL_CODE, (I18nUtil.getString("system_unvalid")));
